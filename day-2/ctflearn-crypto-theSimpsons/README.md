# 🧩 Challenge

## 📌 Basic Info
- **Name**: The Simpsons
- **Platform**: ctflearn
- **Category**: Cryptography
- **Difficulty**: Hard
- **Points**: 60

---

## 📖 Description
Ya know, I was thinking... wouldn't the Simpsons use octal as a base system? They have 8 fingers... Oh, right! The problem! Ummm, something seems odd about this image... https://mega.nz/#!yfp1nYrQ!LOz_eucuKkjAaDqVvz3GWgbfdKWn8BhussKZbx6bUMg

---

## 🔗 Resources
- Link: https://ctflearn.com/challenge/160
- File: https://mega.nz/#!yfp1nYrQ!LOz_eucuKkjAaDqVvz3GWgbfdKWn8BhussKZbx6bUMg ( [ItsKrumpingTime.jpg](ItsKrumpingTime.jpg) )

---

## 🎯 Objective
- Find the flag hidden in the provided image.

---

# 🧠 Solution

## 🔍 Analysis
The real puzzle hidden in the image strings. Getting those puzzle, then we can proceed to work on the solution.

---

## 🛠️ Approach
1. Get the puzzle from the image with `strings` command.
```
Ahh! Realistically the Simpsons would use octal instead of decimal!
encoded = 152 162 152 145 162 167 150 172 153 162 145 170 141 162
key = chr(SolutionToDis(110 157 167 040 155 165 143 150 040 144 151 144 040 115 141 147 147 151 145 040 157 162 151 147 151 156 141 154 154 171 040 143 157 163 164 077 040 050 104 151 166 151 144 145 144 040 142 171 040 070 054 040 164 157 040 164 150 145 040 156 145 141 162 145 163 164 040 151 156 164 145 147 145 162 054 040 141 156 144 040 164 150 145 156 040 160 154 165 163 040 146 157 165 162 051))
key = key + key + chr(ord(key)-4)
print(DecodeDat(key=key,text=encoded))
```
2. `encoded` is the octal representation of the decrypted flag.
3. The sequence number inside SolutionToDis is the next puzzle
```
How much did Maggie originally cost? (Divided by 8, to the nearest integer, and then plus four)
```
Which the answer is 110.

4. Construct the key with the answer.

5. Now we have the key and the decrypted flag. Through a little bit of guessing, it is decrypted using Vigenere cipher.

---

## 💻 Exploit / Code
[solver.py](solver.py)

---

## 🚩 Flag
CTFlearn{wearenumberone}

---

## 📚 Lessons Learned
- Vigenere cipher
- Octal representation

---

## 🧷 Notes
- The puzzle is hidden in the image, so we need to use `strings` command to get it.
- The puzzle is a simple math problem, which the answer is 110.